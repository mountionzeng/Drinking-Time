...
describe("selectExtractedFrameCandidates", () => {
  ...
  it("returns the nearest valid candidate on each side with chronological pairs", () => {
    const result = selectExtractedFrameCandidates({ frames, start });
    expect(result.map(candidate => [candidate.side, candidate.frame.imageId])).toEqual([
      ["left", 1],
      ["right", 3],
    ]);
    expect(result[0]?.pair.left.imageId).toBe(1);
    expect(result[0]?.pair.right.imageId).toBe(2);
  });

  it("does not expose same-time or sub-second candidates", () => {
    expect(
      selectExtractedFrameCandidates({
        start,
        frames: [start, { id: "same", imageId: 9, atMs: 4_000 }, { id: "near", imageId: 8, atMs: 4_900 }],
      })
    ).toEqual([]);
  });
});
...
describe("selectExtractedFrameCandidate", () => {
  ...
  it("returns only the nearest usable candidate on either side", () => {
  ...
  it("rejects same-time candidates and invalid or too-short endpoints", () => {
    expect(
      selectExtractedFrameCandidate({
        frames: [
          { id: "start", imageId: 1, atMs: 2_000 },
          { id: "same", imageId: 2, atMs: 2_000 },
        ],
  ...
  it("normalizes a reverse selection into chronological endpoints", () => {
    const result = selectExtractedFrameCandidate({
      frames,
      start: frames[2],
      atMs: 1_100,
    ...
    expect(result.kind).toBe("ok");
    if (result.kind === "ok") {
      expect(result.candidate).toEqual(frames[0]);
      expect(result.pair.left).toEqual(frames[0]);
      expect(result.pair.right).toEqual(frames[2]);
    }
  });
});
