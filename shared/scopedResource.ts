 ...
 * `activeStoryId === xxx.storyId` 这类零散比较表达同一件事。真正的资源级
 * CAS 持久化在 U3 落地；这里只固定合同形状，方便 U3/U4/U5/U7/U8 复用同一
 * 统计口径和判定逻辑。
 */
 ...
 * 调用入口（尚未接入，U3 落地）：router/service 在执行任何 Story/Publishing
...
/**
 * 客户端专用的缓存分区键——附加 `cacheUserId` 只是为了同一浏览器多账号场景
 * 下隔离 query cache，绝不能被服务端当作鉴权输入使用。
 */
export type ClientCacheScopeKey = ScopeKey & { cacheUserId: number };

/**
 * 用途：构造客户端 query cache 分区键，把 `cacheUserId` 附加到资源 scope 上。
 * 调用入口（尚未接入，U7 落地）：client `lib/trpc.ts` 构造 query/mutation key 时。
 * 下游调用：react-query 的 queryKey；不进入任何服务端调用参数。
 */
export function deriveClientCacheScopeKey(
  scope: ScopeKey,
  cacheUserId: number
): ClientCacheScopeKey {
  return { ...scope, cacheUserId };
}
 ...
 * 调用入口（尚未接入，U3 落地）：router 输入校验层、`parseDomainCommand`。
 ...
 *   `currentStoryScopeKey` 目前只导出 ScopeKey，尚未接入本函数（U7 落地）。
 * 下游调用：无（叶子纯函数）。
 */
 ...
 * 调用入口（尚未接入，U3 落地）：router mutation 输入校验层。
 ...
 * 调用入口：领域 persistence 模块的资源写入成功分支（U3 落地）。
 ...
 * 调用入口（尚未接入，U3 落地）：领域 persistence 模块在写入资源 B 后，为
 *   资源 A 的聚合投影调用。
 * 下游调用：无（叶子纯函数）。
 */
 ...
 *   里的唯一判定点。
 * 调用入口：领域 persistence 模块的资源写入前置校验（U3 落地）。
 * 下游调用：无（叶子纯函数）。
 */
