...
export function useProjectData() {
  ...
  // 当前故事（U4）。唯一真相源与持久化在 StoryAgentContext（它有 hydrate / 选择屏置 null
  // 等逻辑并自持久化）；这里只作纯中转，由 StoryAgentProvider 经 onActiveStoryChange 向上同步。
  // 不在此处持久化——否则 StoryAgent 挂载时 emit 的初始 null 会反过来抹掉缓存（评审 P1）。
