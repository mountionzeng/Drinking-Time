...
import type {
  PromptFragment,
  FragmentTag,
} from "@/features/storyAgent/promptPool";
  ...
  return (
    <div className="flex gap-1.5 items-baseline">
      ...
      <span
        ...
        data-stable-shot-id={stableShotId ?? undefined}
        data-shot-no={shotNo ?? undefined}
        data-selection-version={selectionVersion ?? undefined}
        contentEditable
      ...
      {open && (
        ...
        >
          <input
            ...
            autoFocus
          />
          {(Object.entries(grouped) as [FragmentTag, PromptFragment[]][]).map(
            ([tag, frags]) =>
              frags.length > 0 ? (
        ...
        {referenced.map(f => (
          <FragmentBadge
            ...
            onRemove={() => {
              onUpdateRefs(
                shotIndex,
                fragmentRefs.filter(id => id !== f.id)
              );
            }}
          />
        ))}
...
export default function ShotTable({
  ...
  const [sortBy, setSortBy] = useState<
    "scene" | "priority" | "deadline" | "readiness"
  >("scene");
        ...
        <>
          ...
          >
            <select
              ...
              value={filterStatus}
              onChange={e =>
                setFilterStatus(e.target.value as ShotStatus | "all")
              }
            >
                ...
                {filteredShots.map(shot => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-2.5 py-2.5 font-mono text-nayin">
                        {shot.shotNo}
                      </td>
                      {onShotClick && (
                        <td
                          ...
                          onDrop={e => {
                            ...
                            try {
                              ...
                              if (
                                parsed.fromShotNo !== shot.shotNo &&
                                parsed.imageId
                              ) {
                                onShotClick(shot.shotNo);
                                window.dispatchEvent(
                                  new CustomEvent("dt:reassign-image", {
                                    detail: {
                                      imageId: parsed.imageId,
                                      newShotNo: shot.shotNo,
                                    },
                                  })
                                );
                              }
                      ...
                      <td className="px-2.5 py-2.5">
                        <div className="flex items-center gap-2.5 min-w-[112px]">
                          ...
                          <span
                            ...
                            style={{
                              background:
                                statusConfig?.bgColor ?? "var(--muted)",
                          ...
                          >
                            <span
                              ...
                              style={{
                                background:
                                  statusConfig?.color ?? "var(--foreground)",
                              }}
                            />
                          ...
                          {raw && srcIdx != null ? (
                            ...
                            >
                              <EditableLine
                                ...
                                onCommit={v =>
                                  onEditShotField?.(srcIdx, "subject", v)
                                }
                                selectionSource={`shot:${srcIdx}:subject`}
                                storyId={storyId}
                                stableShotId={
                                  raw.stableShotId ?? raw.shotIdentity
                                }
                                shotNo={raw.shotNo}
                                selectionVersion={`shot:${raw.stableShotId ?? raw.shotIdentity ?? srcIdx}:subject`}
                              />
                              <EditableLine
                                label="动作"
                                value={raw.action}
                                placeholder="（发生了什么）"
                                onCommit={v =>
                                  onEditShotField?.(srcIdx, "action", v)
                                }
                                ...
                                shotNo={raw.shotNo}
                                selectionVersion={`shot:${raw.stableShotId ?? raw.shotIdentity ?? srcIdx}:action`}
                              />
                              <EditableLine
                                label="台词"
                                value={raw.dialogue}
                                placeholder="（无台词）"
                                onCommit={v =>
                                  onEditShotField?.(srcIdx, "dialogue", v)
                                }
                                ...
                                shotNo={raw.shotNo}
                                selectionVersion={`shot:${raw.stableShotId ?? raw.shotIdentity ?? srcIdx}:dialogue`}
                              />
                            </div>
                          ) : (
