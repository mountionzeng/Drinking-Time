 ...
 *
 * 线稿分成「身体」和「热气」两层：EmotiveWuxingIcon 要单独给热气挂
 * hover 飘动的动画，所以热气必须能拿到自己的 <g className="dt-steam">。
...
function BeerMugSteam() {
  return (
    <g className="dt-steam">
...
function CoffeeMugBody() {
  return (
    <>
      ...
      <path d="M22,72 L68,72" strokeWidth=".7" opacity=".4" />
      <ellipse
        cx="45"
        cy="32"
        rx="25"
...
/** 每个元素的饮品线稿（身体 + 热气），供 EmotiveWuxingIcon 等组件叠加表情时复用。 */
...
/** 每个元素线稿使用的墨色，叠加层（手脚）沿用同色保持手绘一致感。 */
...
/** 五官墨色：深色身体（火 / 土）比线稿再深一档，靠深浅站住，不垫浅色脸底。 */
export const WUXING_FACE_INK: Record<NayinElement, string> = {
  metal: "#7A5B1F",
  wood: "#33532B",
  water: "#3E6D7D",
  fire: "#5A211A",
...
export default function WuxingDrinkIcon({
  element,
  size = 36,
  className = "",
}: Props) {
  ...
  return (
    ...
    >
      <svg
        viewBox="0 0 90 100"
        fill="none"
        stroke={WUXING_DRINK_INK[element]}
        color={WUXING_DRINK_INK[element]}
        strokeLinecap="round"
