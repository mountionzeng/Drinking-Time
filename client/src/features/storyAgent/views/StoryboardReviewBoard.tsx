  ...
  const generateThirtySecondVideo = async () => {
    ...
    const plan = planStoryboardOneClickVideo(
      ...
      30,
      {
        ...
        estimatedImageCny: imageEstimate.estimatedCny,
      }
    );
        ...
        <div className="flex items-center gap-2">
          ...
          {shots.length > 0 && onGenerateShotVideo ? (
            <button
              ...
              disabled={Boolean(oneClickVideoProgress) || !onPromoteFrameCrop}
                      ...
                      >
                        ...
                        {onGenerateShotImages ? (
                          <button
                            ...
                            aria-label={`根据正式封面、前后画面和图片要求重新生成 ${shotLabel} 图片`}
                            title={
                              imageProviderStatus?.ready
                                ? "继承正式封面，并根据前后画面和图片要求重新生成图片"
                          ...
                          action={
                            ...
                            onGenerateShotImages ? (
                              <button
                                ...
                                aria-label={`继承正式封面并按图片要求渲染 ${shotLabel} 的四张候选图`}
                                title={
                                  imageProviderStatus?.ready
                                    ? "原文要求优先，并继承文字稿正式封面生成四张同风格候选图"
                                    : (imageProviderStatus?.reason ??
                                      "正在确认图片供应商状态")
                                }
                              >
                              ...
                              <button
                                ...
                                aria-label={`继承当前主图和正式封面，按视频要求渲染 ${shotLabel} 视频`}
                                title={
                                  matrixVideoBlockReason ??
                                  "当前主图作为首帧、正式封面作为风格参考；保存文字并确认费用后生成候选 Take"
                                }
                              >
                                {continuityCheckingByShot[shot.shotNo] ===
                                "video" ? (
