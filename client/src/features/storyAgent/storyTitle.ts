...
const MAX_AUTOMATIC_TITLE_LENGTH = 24;
...
function compactTitle(value: string): string {
  const compact = value
    ...
    .replace(/^[《「『“"]|[》」』”"]$/g, "")
    .trim();
  if (compact.length <= MAX_AUTOMATIC_TITLE_LENGTH) return compact;
  return compact.slice(0, MAX_AUTOMATIC_TITLE_LENGTH);
}
...
function quotedIdeas(messages: readonly string[]): string | null {
  for (const message of [...messages].reverse()) {
    ...
    if (unique.length >= 2) {
      return compactTitle(`${unique[0]}与${unique[1]}`);
    }
    if (unique.length === 1 && unique[0]!.length >= 4) return unique[0]!;
  }
  return null;
}
...
function conversationPhrase(messages: readonly string[]): string | null {
  const candidates = messages
    .flatMap((message, messageIndex) =>
      message
        .replace(
          /^(?:我(?:突然)?(?:想到|想起)(?:一个|了)?(?:事儿|事情)?|我觉得|我感觉|其实|就是|最近|今天)[，,：:\s]*(?:就是)?[，,：:\s]*/,
          ""
        )
        .split(/[。！？!?；;，,\n]+/)
  ...
  return candidates[0]?.phrase ?? null;
}
