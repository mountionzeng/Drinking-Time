...
export function PublishingAlbumPagePreview({
  ...
  return (
    <div
      className="relative aspect-[3/4] w-full overflow-hidden rounded-xl bg-black/10"
      aria-label={label}
    ...
    >
      {backgroundUrl ? (
        ...
        <div className="absolute inset-0 grid place-items-center text-xs text-muted-foreground">尚未采用底图</div>
      )}
      {plan ? (
        <svg
        ...
        >
          ...
          {plan.graphemes.map(glyph => glyph.grapheme === "\n" ? null : (
              <text
                ...
                transform={glyph.rotation ? `rotate(${glyph.rotation} ${glyph.x} ${glyph.y})` : undefined}
              >
                {glyph.grapheme}
              </text>
            ))}
      ...
      {candidate ? (
        <span className="absolute left-2 top-2 rounded bg-black/65 px-2 py-1 text-[10px] text-white">候选 · 尚未采用</span>
      ) : null}
    </div>
  );
}
