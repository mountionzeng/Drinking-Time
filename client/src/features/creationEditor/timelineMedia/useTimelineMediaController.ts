...
export async function handleMediaCommandResult(
  ...
  if (!result.receipt) {
    const error = "媒体操作缺少可撤销回执，请刷新后重试";
    deps.setError(error);
    await deps.onChanged();
    return { recorded: false, refetched: true, error };
  }
...
export type TimelineMediaController = {
  /** The persisted slice is authoritative even when it contains no cues. */
  subtitleSlicePresent: boolean;
  ...
  // ── Audio (U9) ──────────────────────────────────────────────────────
  /** The persisted slice is authoritative even when all five tracks are empty. */
  audioSlicePresent: boolean;
...
export function useTimelineMediaController(
  ...
  const subtitleSlicePresent = Object.prototype.hasOwnProperty.call(
    extensions ?? {},
    "subtitleTracks"
  );
  const audioSlicePresent = Object.prototype.hasOwnProperty.call(
    extensions ?? {},
    "audioTracks"
  );
  ...
  const quoteSceneAudioMut = trpc.timelineMedia.quoteSceneAudio.useMutation();
  const generateSceneAudioMut =
    trpc.timelineMedia.generateSceneAudio.useMutation();
  ...
  return {
    ...
    editSubtitleText: useCallback(
      ...
      [runForResult, editMut, storyId, operation]
    ),
    ...
    generateSceneAudio: useCallback(
      async inputArgs => {
        if (storyId == null) {
          setLastError("故事尚未加载，无法生成声音");
          return false;
        }
        setPendingCount(count => count + 1);
        setLastError(null);
        const commandSessionKey = renderedSessionKey;
        try {
          const quote = await quoteSceneAudioMut.mutateAsync({
            storyId,
            kind: inputArgs.kind,
          ...
          if (sessionKeyRef.current !== commandSessionKey) return false;
          const kindLabel =
            inputArgs.kind === "music"
              ? "音乐"
              : inputArgs.kind === "ambience"
                ? "环境声"
                : "音效";
          ...
          const result = await trackCreationEditorOperation(
            storyId,
            generateSceneAudioMut.mutateAsync({
              storyId,
          ...
          if (sessionKeyRef.current !== commandSessionKey) return false;
          setLastError(error instanceof Error ? error.message : "声音生成失败");
          return false;
        } finally {
          if (sessionKeyRef.current === commandSessionKey) {
            setPendingCount(count => Math.max(0, count - 1));
    ...
    generateNarration: useCallback(
      async (subtitleCueId: string) => {
        ...
        try {
          ...
          if (sessionKeyRef.current !== commandSessionKey) return false;
          setLastError(error instanceof Error ? error.message : "旁白生成失败");
          return false;
        } finally {
          if (sessionKeyRef.current === commandSessionKey) {
            setPendingCount(count => Math.max(0, count - 1));
