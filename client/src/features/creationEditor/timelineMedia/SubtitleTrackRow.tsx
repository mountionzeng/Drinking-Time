...
export function shouldSubmitSubtitleBlur(composing: boolean): boolean {
  return !composing;
}
...
export function SubtitleTrackRow({
  ...
  const submitEdit = useCallback(async () => {
    ...
    setStatus("正在保存字幕…");
    const saved = await binding.onEditText({
      cueId: editingCue.id,
      text: next,
      expectedTextRevision: editingCue.textRevision,
    });
    setStatus(subtitleSaveStatus(saved));
  ...
  return (
    ...
    >
      {cues.length === 0 ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5 text-[9px] text-muted-foreground/80">
          ...
          {binding.error ? (
            <span
              role="alert"
              data-testid="storyboard-subtitle-error"
              className="max-w-full truncate px-2 text-[8px] text-destructive"
              title={binding.error}
            >
              {binding.error}
            </span>
          ...
          >
            {editing ? (
              <textarea
                ...
                onBlur={() => {
                  if (shouldSubmitSubtitleBlur(composing)) void submitEdit();
                }}
              />
            ) : (
              <>
