...
function StoryboardExtractedFrameRows({
  ...
  const [pairingStart, setPairingStart] = useState<{ id: string; imageId: number; atMs: number } | null>(null);
  const [pairingCandidate, setPairingCandidate] = useState<ExtractedFrameCandidateResult | null>(null);
      ...
      >
        <div
          ...
          onPointerMove={event => {
            ...
            const rect = event.currentTarget.getBoundingClientRect();
            const atMs = Math.max(0, Math.min(totalMs, ((event.clientX - rect.left) / Math.max(1, rect.width)) * totalMs));
            const result = selectExtractedFrameCandidate({ frames, start: pairingStart, atMs });
            setPairingCandidate(result);
          }}
          onPointerLeave={() => {
        ...
        >
              ...
              <button
                ...
                onClick={() => {
                  if (pairingStart) {
                    ...
                    const candidate = selectExtractedFrameCandidates({ frames, start: pairingStart }).find(item => item.frame.id === frame.id);
                    if (candidate) void finishPairing({ leftImageId: candidate.pair.left.imageId, rightImageId: candidate.pair.right.imageId });
                    return;
                  }
          ...
          {pairingStart && pairingCandidate?.kind === "ok" ? (
            <button
              type="button"
              aria-label={`选择候选抽帧 ${formatStoryboardTimestamp(pairingCandidate.pair.right.atMs)}`}
              ...
              style={{ left: `${(pairingCandidate.candidate.atMs / Math.max(1, totalMs)) * 100}%`, top: "0.1rem" }}
              onClick={event => {
                event.stopPropagation();
                void finishPairing({ leftImageId: pairingCandidate.pair.left.imageId, rightImageId: pairingCandidate.pair.right.imageId });
              }}
            >
...
function StoryboardEditTrack({
  ...
  const endSingleDrag = async (event: ReactPointerEvent<HTMLElement>) => {
    ...
    setSingleDrag(null);
    if (!start || !drag || drag.deltaFrames === 0 || drag.blockedReason) return;
  ...
  return (
    ...
    >
      ...
      data-testid="storyboard-edit-track"
        className="relative h-14 w-full cursor-text touch-none rounded-sm border border-border/70 bg-muted/30 outline-none focus-visible:ring-2 focus-visible:ring-primary/35"
              ...
              .map(block => {
                ...
                return (
                  <span
                    ...
                    className="pointer-events-none absolute bottom-0.5 top-0.5 z-30 rounded-[2px] border-2 border-dashed border-primary bg-primary/20"
                    style={{
