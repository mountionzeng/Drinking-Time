...
describe("creation editor prompt persistence", () => {
  it("writes duration into the target shot without changing sibling shots", () => {
    ...
    expect((next.shots as any[])[1]).toMatchObject({
      shotNo: 2,
      durationMs: 4200,
    });
  ...
  it("overwrites editable shot content before saving downstream prompt metadata", () => {
    const next = writeShotContentSnapshot(
      {
        shots: [
          {
            shotNo: 2,
