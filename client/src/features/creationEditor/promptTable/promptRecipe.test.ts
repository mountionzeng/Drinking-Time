...
describe("promptRecipe", () => {
  it("compiles weighted rows into the final generation prompt", () => {
    ...
    expect(recipe.finalPrompt).toContain(
      "Create exactly one cinematic key frame for SH01"
    );
    expect(recipe.finalPrompt).toContain(
      "single continuous shot frame, not a storyboard sheet"
    );
    expect(recipe.finalPrompt).toContain("Source Story Card: 他站在门口。");
    expect(recipe.finalPrompt).toContain(
      "Shared visual framework for the whole film: premium commercial film, off-white"
    );
    expect(recipe.finalPrompt).toContain(
      "Single-frame rule: compose one uninterrupted cinematic camera frame only"
    );
    expect(recipe.finalPrompt).toContain(
      "no collage, no contact sheet, no inset thumbnails"
    );
    expect(recipe.finalPrompt).toContain(
      "No captions, no readable text, no UI, no watermark"
    );
    expect(recipe.finalPrompt).toContain(
      "Dialogue/subtitle/voiceover reference: 我要开始了"
    );
    expect(recipe.finalPrompt).toContain(
      "Do not render the dialogue as visible captions"
    );
