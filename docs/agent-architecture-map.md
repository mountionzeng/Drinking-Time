...
**C. 死代码清查已执行（2026-06-13）：**
- ❌ 已删：10 个无客户端调用的 `/api/archive/*` 路由（_core/index.ts）+ 独占模块 `dropZoneAgent.ts`、`analysisShell.ts` + 配套死助手（getRequestUserId/pickStoryPatch 等）。共删 ~1100 行。**顺带消除了 analysis-shell 硬编码 userId=1、dropZoneAgent 无 userId 过滤的越权隐患。**
- ✅ 保留（仍是载重代码）：`storyReply.ts`（`replyFromStoryAgent`/`deriveMobileImagePrompt` 被 **routers.ts 活用**——地图初稿误判为死，已更正）、`visionAgent.ts`（活的 artAgent）、`shotSynthesis`、`summary`、`selectionEdit`、`storyAgent.*` 枢纽。
- 🔜 `storyIntent.ts`（意图分类器，含 linkedin_job_search 规则）**保留待复活**——下一步"自动识别意图"用它。

> 教训：`archive/` 名字会骗人，删之前**必须用 grep 追实际引用**（含经 `storyAgent` 枢纽的间接引用），不能按目录名删。
