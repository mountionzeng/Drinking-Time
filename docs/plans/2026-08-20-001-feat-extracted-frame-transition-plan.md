...
在故事版的“抽帧 · 上层”轨道中，用户右键两张抽帧之间的空白区域，系统用点击位置左右最近的两张抽帧作为首帧和尾帧，先在聊天中生成一张带真实价格的待确认卡；只有用户确认后才提交付费生成。生成结果从左侧抽帧的绝对时间开始摆放为持久上层覆盖片段，并原子创建对应的 story shot 与 timeline item；原有底层镜头和素材不删除。

## 最终语义修订（2026-08-20）

本文下方保留了最初获批的 B 方案及其实施过程，其中大量段落写着“只创建 Timeline overlay、不创建 story shot”。当天后续验收中，用户明确批准把采用语义改为：**同一原子操作创建 overlay、对应 story shot、timeline item，并绑定生成 Take**，这样完整故事板会出现可编辑的真实镜头列。当前权威事实以 `extracted-frame-overlay-video` 功能卡、`applyStoryTimelineOverlayAtomic` 和相应测试为准；下方相反表述仅用于保存历史决策来路，不得作为下一轮实现要求。
...
- 已完成持久 Timeline overlay、对应 story shot 与 timeline item 的原子采用、完整媒体播放、尾段显式 gap、锚点优先，以及预览/剪辑线/导出统一解析。
...
- 最终自动化验证（提交前复核）：全仓 `309/309` 个测试文件、`2505/2505` 条已执行测试通过，另有 5 条按既有环境条件跳过；TypeScript、生产构建、功能账本和 `git diff --check` 均通过。
- 初次未付费验收时，story `#1186` 为 30 镜、timeline 30 项、原 `0102` 存在，持久 overlay 数为 0。后续已用付费 Take 1596 验收最终采用语义：幂等重放补齐历史缺失的 story shot 与 timeline item，且没有重复供应商任务。
