...
| MJ 镜头渲染 | `codex/mj-shot-render` / `.worktrees/codex/mj-shot-render` | shared/shotImageRender、useShotImageRender、参考按钮、storyAgent图片路由、imageGen和资产上下文及测试/账本 | 按用户要求改回MJ，按四图任务报价，保留显式参考；不付费生成，不启动worktree服务 | 2026-09-09 |
（收工时删掉自己这行。）
