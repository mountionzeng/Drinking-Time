...
| 刷新延迟修复 | `claude/story-refresh-latency`（worktree `.worktrees/story-refresh-latency`） | Phase A: `client/src/features/creationEditor/CreationEditorContext.tsx`（拆分 initialStoryLoading / enhancement loading，不改 `saveTimelineItems`/整份 timeline 写入口）、`client/src/features/creationEditor/views/EditingNleWorkspace.tsx`（loading 门判断）；Phase B: 同两文件去重首屏 `storyList`/`storyGet`；Phase C: `server/services/promptLineageStore.ts`、`server/services/storyMaterials.ts`、`server/db.ts`（新增按 Story 窄读选择器，不改现有导出签名） | **进行中**，按 docs/handoff/2026-08-25-story-refresh-latency-handoff.md 的 Phase A→B→C 顺序 | 2026-08-25 |
（收工时删掉自己这行。）
