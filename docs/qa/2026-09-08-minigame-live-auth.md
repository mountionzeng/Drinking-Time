...
状态：**可编辑手机版工作区后端已部署测试站；2026-09-09 用户明确批准后，含微信登录的新预览包上传成功并展示二维码；真实微信登录未验收，不可正式发布**。下方早期记录按时间保留，以本节为当前状态。
...
- 模拟器显示邮箱/验证码/微信登录页。基础库启动曾报jsbridge not ready，随后登录页正常绘制；未进行真实wx.login或旧账号绑定，不把预览成功当成完整功能验收。
- 测试站只读探针ready/mysql/required、匿名stories401正常；未部署、未改服务器配置或业务数据。建议原邮箱登录→关联当前微信→退出→微信登录→核对原故事。
