...
| 文件 | owner | 原因 | 到期条件 |
| --- | --- | --- | --- |
| `server/persistence/shiguangStoryImportPersistence.ts` | 拾光家忆故事进入电脑 | 微信端上传的是不可变故事快照，导入必须先按账号、来源和修订幂等查询，再创建归属同一账号的新故事。该文件是这组落库操作的唯一 persistence seam，桥接路由和领域服务均不直接访问 `db.ts`。 | 幂等查询与故事创建物理迁出 `db.ts`、进入独立 repository package 时整体迁移本条；不得把直接导入散回桥接路由或导入服务。 |
| `server/services/accountIdentity.ts` | 统一账号·赠送卡·算力账本线（U4） | 统一账号是另一个新领域，同样没有可复用的 persistence。验证码挑战的「读取 → 比对 → 计数或标记已用」必须在同一个事务里完成，否则同一个码能被并发用两次；限流的「读窗口 → 判断 → 写回」同理。判断逻辑（密码策略、摘要算法）全在无副作用的 `accountSecurity.ts` 里，本文件只做编排。 | 与 `computeLedger.ts` 同理，是新领域 seam 而非临时欠债。到期条件同样是**收敛**：U4/U5 落地后若账号读写全部经由本文件，转为常规基线条目；若出现第二个直接导入 `db` 的账号文件，必须合并回本文件，不允许再加一条豁免。 |
| `server/services/computeLedger.ts` | 统一账号·赠送卡·算力账本线（U6） | 算力账本是一个新领域，仓库里没有可复用的 persistence：余额投影、预占、结算和账本追加都必须在 `db.transaction` 的锁内完成，隔一层就失去原子性。本文件就是这个领域的 persistence seam，判断逻辑全在纯函数 `computeBilling.ts` 里，本文件只做落库编排。 | 本条是「新领域 seam」而不是临时欠债，不会自动失效。真正的到期条件是**收敛而非删除**：U6–U8 落地后，若账务读写全部经由本文件，把本行从豁免表转成常规基线条目；若出现第二个直接导入 `db` 的账务文件，必须合并回本文件，不允许再加一条豁免。 |
