...
- R13.1 **唯一写入口必须同时定义「它拒绝时人怎么接管」**。命令因版本冲突、边界校验或状态不确定而失败时，界面必须给出可见原因和一条可执行的出路（重试、以服务端最新状态刷新后重来、或明确的人工裁决入口）；不允许静默 return、静默回弹，也不允许把用户锁在一个自己无法解除的状态里。
  依据：视觉资产标准板线的实战教训——它同样立了「标准视图 status 只能由结构质检写」的唯一入口不变量，结果质检判 `unknown` 时锁定按钮永久置灰，而唯一能改判的 `reviewViews` 接口没有 UI 入口，用户花了钱却被系统告知不能用、且无处申诉。**不变量成立、用户被锁死在外面，是收敛失败而不是成功。**
