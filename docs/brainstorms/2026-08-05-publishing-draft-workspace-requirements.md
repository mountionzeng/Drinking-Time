...
- F4. 从发布稿进入封面和视频创作
  ...
  - **Actors:** A1, A2, A4
  - **Steps:** 系统基于故事内核与当前稿生成四张封面候选 → 用户迭代并明确采用一张故事级主视觉 → 用户点击进入视频制作 → 系统读取当前发布版本并直接打开故事版 → 按非空段落生成带来源标记的未确认剧本与逐镜草案 → 用户在故事版内检查、修改和确认 → 确认后一次性采用为正式镜头 → 后续镜头图片统一参考正式封面。
  - **Outcome:** 用户不经过重复预览页即可在真实工作界面检查草案；原文字稿完整保留并能追溯到剧本和镜头，未经确认的草案不会污染正式故事版或旧媒体。
...
- R17. 发布稿是视频剧本的上游来源而不是临时面板；用户在工作区之间切换时不得清空发布稿、剧本或对应关系。只有用户明确点击「进入视频制作」才生成草案并直接打开故事版，普通工作区切换不得自动改写或产生模型调用。
...
- R23. 新版本必须保留旧版本故事内核、平台稿、正式封面和视频交接材料的来源关系；不得自动生成新封面、重写旧平台稿或触发付费模型调用。旧版正式封面进入新版本时只能作为标明来源的临时美术基线，并显示“待重新确认”；用户确认或采用新封面前，不得把它冒充为新版本的正式封面。
...
- R28. 用户点击「进入视频制作」后，系统必须直接打开故事版，并在故事版内展示一份与正式镜头清楚区分的未确认剧本与镜头草案；不得增加独立预览页，未经用户在故事版内确认，草案不得替换或新增正式镜头。
...
- AE7. **Covers R16, R17, R28.** Given 用户已经完成 X 稿和封面，when 用户点击「进入视频制作」，then 发布稿状态继续保留，系统直接打开故事版并展示未确认剧本与镜头草案；when 用户只是切换到其他工作区，then 不生成或改写剧本。
...
- AE13. **Covers R23, R24.** Given V1 已有小红书稿、X 稿和正式封面，when 用户确认创建 V2，then V2 只把 V1 封面显示为“来自 V1、待重新确认”的临时美术基线，只生成当前平台的新稿，X 稿显示「待更新」，且没有后台付费调用。
- AE14. **Covers R25, R26.** Given 故事已有 V1 和 V2，when 用户在顶部切换版本，then 对话上下文、平台稿、封面和视频交接材料都切换到对应版本；在 V2 中修改或换封面不会改变 V1。
- AE15. **Covers R27, R29, R30, R31.** Given `#1172` 当前发布稿有六个非空正文段落且故事版只有一个封面占位镜头，when 系统在故事版内生成未确认草案，then 出现至少六个可追溯的剧本段落和对应镜头，每个原文段落都能定位到自己的转写结果，且话题标签不进入旁白。
- AE16. **Covers R28, R32.** Given 用户正在故事版内检查六镜未确认草案，when 用户取消或保存失败，then 正式故事版保持原状；when 用户确认且保存成功，then 六个镜头和对应剧本同时被采用，不存在只写入前三镜的状态。
...
- AE31. **Covers R23, R53, R55.** Given V1 已有正式封面、已确认故事版、采用图片和多个视频 Take，when 创建并切到 V2，then 页面把 V1 封面标为待重新确认的临时基线，并清楚标明其他材料哪些来自 V1、哪些待更新；V1 资产保持可用，V2 不会自动把它们改写为自己的已确认成果。
...
- 普通工作区切换不自动拆镜；只有用户明确点击「进入视频制作」才直接打开故事版并生成未确认草案。
...
- 直接进入故事版、原位确认后原子采用：用户点击「进入视频制作」后不经过重复预览页，未确认草案直接出现在故事版；正式镜头仍只有在用户确认后才改变。
...
- 旧版封面只作为新版临时基线：新版本保持视觉连续性，但继承封面必须标明来源并等待重新确认，不能因为复制成功就获得新版正式身份。
...
- [Affects R28, R32, R34][Technical] 故事版内未确认草案、整组采用和已有媒体保护如何形成可恢复的原子操作，并在新旧镜头结构变化时给出可靠影响说明？
