...
async function runMediaCommand(
  input: {
    ...
    /**
     * A provider job that was accepted while the epoch was active may finish
     * after the editor moved on. Only that server-owned continuation may
     * complete its already-authorized Timeline write.
     */
    allowRetiredSession?: boolean;
  },
  planner: (
    slices: {
  ...
  return withVisualEditServiceLock(input.storyId, input.userId, async () => {
    ...
    if (replay) {
      ...
      if ("replayOnly" in replay) {
        const current = await loadOwnedStoryTimelineEnvelope({
          storyId: input.storyId,
          userId: input.userId,
        });
        return {
          status: "ok",
          timelineVersion: current?.version ?? replay.afterTimelineVersion,
          changed: false,
        };
      }
      return {
    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      const durableReplay = findDurableTimelineMediaOperation(
        envelope.extensions,
        input.operation
      );
      if (durableReplay) {
        if (durableReplay.commandDigest !== commandDigest) {
          return {
            status: "error",
            error: "操作标识已用于另一条命令",
            errorKind: "invalid",
      ...
      try {
        const nextExtensions = appendDurableTimelineMediaOperation(
          { ...envelope.extensions, ...planned.nextExtensions },
          {
            ...input.operation,
            commandDigest,
          }
...
async function insertAudioClipForStoryWithSessionPolicy(
  ...
  const assetDurationFrames = Math.max(1, asset.durationFrames ?? 1);
...
export async function insertAudioClipForStory(
  ...
  return insertAudioClipForStoryWithSessionPolicy(input, false);
}

/**
 * Completes a paid provider job that passed the router's session guard before
 * submission. This is intentionally not exposed as a router command: callers
 * cannot use it to start a new edit from a retired session.
 */
export async function insertGeneratedAudioClipForStory(
  input: InsertAudioClipCommand
): Promise<TimelineMediaCommandResult> {
  return insertAudioClipForStoryWithSessionPolicy(input, true);
}
...
export async function adoptNarrationCandidateForStory(input: {
  ...
  return runMediaCommand(
    ...
    async ({ audio, subtitle }, ids) => {
      // This ready/ownership check deliberately runs inside the shared Story
      // edit lock. Candidate discard uses the same lock, so it cannot mark the
      // asset failed and remove its bytes between this check and the CAS.
      const asset = await loadReadyStoryAudioAsset({
        scope: { storyId: input.storyId, userId: input.userId },
