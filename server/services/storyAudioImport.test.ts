...
describe("SSRF guard — assertAllowedRemoteAudioUrl", () => {
  it("accepts an HTTPS S3 url on 443", async () => {
    ...
    expect(() =>
      assertAllowedRemoteAudioUrl(
        "https://my-bucket.s3.us-west-2.amazonaws.com/a.mp3"
      )
    ).not.toThrow();
  });
  ...
  it.each([
    ...
    const { assertAllowedRemoteAudioUrl, UnsafeRemoteAudioError } =
      await importSvc();
    try {
...
describe("local import capacity", () => {
  it("allows one import per user and two globally without queuing excess work", async () => {
    const { acquireLocalAudioImportPermit } = await importSvc();
    const releaseA = acquireLocalAudioImportPermit(1);
    expect(releaseA).toEqual(expect.any(Function));
    expect(acquireLocalAudioImportPermit(1)).toBeNull();
    const releaseB = acquireLocalAudioImportPermit(2);
    expect(releaseB).toEqual(expect.any(Function));
    expect(acquireLocalAudioImportPermit(3)).toBeNull();
    releaseA?.();
    const releaseC = acquireLocalAudioImportPermit(3);
...
describe("SSRF guard — DNS rebinding", () => {
  it("rejects a hostname that resolves to a private address", async () => {
    const { assertResolvedIpsArePublic, UnsafeRemoteAudioError } =
      await importSvc();
...
describe("fetchTrustedAudioBytes", () => {
  ...
  it("rejects a redirect that points off the allow-list", async () => {
    const { fetchTrustedAudioBytes, UnsafeRemoteAudioError } =
      await importSvc();
...
describe("importAudioBytes state machine", () => {
  ...
  it("replays a settled operation id without importing again", async () => {
    ...
    const { listStoryAudioAssetRows } = await db();
    expect(await listStoryAudioAssetRows({ storyId, userId: 1 })).toHaveLength(
      1
    );
  });

  it("rejects reusing an operation id for different bytes without changing the settled asset", async () => {
    const storyId = await seedStory();
    const { importAudioBytes } = await importSvc();
    const first = await importAudioBytes({
      scope: { storyId, userId: 1 },
      operationId: "op-payload",
      sourceKind: "local-upload",
      displayName: "once",
      bytes: goodWavBytes,
    });
    const conflicting = await importAudioBytes({
      scope: { storyId, userId: 1 },
      operationId: "op-payload",
      sourceKind: "local-upload",
      displayName: "once",
      bytes: Buffer.concat([goodWavBytes, Buffer.from([0])]),
    });
    expect(first.status).toBe("ready");
  ...
  it("rejects operation ids that cannot be used as safe staging keys before writing metadata", async () => {
    const storyId = await seedStory();
    const { importAudioBytes } = await importSvc();
    const result = await importAudioBytes({
      scope: { storyId, userId: 1 },
...
describe("recoverStaleAudioImports", () => {
  it("finalizes a probed import when the managed rename committed before the database update", async () => {
    const storyId = await seedStory();
    const {
    ...
    await createStoryAudioImportOperationRow({
      ...
      sourceKind: "local-upload",
      status: "probed",
      stagingKey: "op-renamed",
    });
    const { ensureManagedAudioDirs, resolveManagedAudioPath } = await import(
      "./audioMedia"
    );
    await ensureManagedAudioDirs();
    await writeFile(resolveManagedAudioPath(storageKey), goodWavBytes);

    const { recoverStaleAudioImports } = await importSvc();
    const report = await recoverStaleAudioImports();
    expect(report.compensatedOperations).toBe(0);
    expect(
      await getStoryAudioImportOperationRow({
        storyId,
        userId: 1,
        operationId: "op-renamed",
      })
    ...
    ).toMatchObject({ status: "ready", checksum: expect.any(String) });
  });
