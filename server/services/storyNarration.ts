...
function narrationProvenance(
  value: unknown
): NarrationCandidateProvenance | null {
...
export async function listStoryNarrationCandidates(input: {
  ...
  if (
    !(await loadOwnedStory({ storyId: input.storyId, userId: input.userId }))
  ) {
    return [];
  }
  ...
  return ready.map(({ asset, provenance }) => {
    ...
    if (!cue) unavailableReason = "字幕已经删除";
      ...
      cue.speechBindingId &&
      cue.speechBindingId !== provenance.bindingId
    ) {
...
export type GenerateStoryNarrationResult =
  | {
      ...
      candidate: StoryNarrationCandidate;
      replayed: boolean;
    }
...
export async function generateStoryNarrationCandidate(input: {
  ...
  let voice: StoryVoice302Result | null = null;
  if (reservation.outcome === "replayed") {
    ...
    const attempts = await dependencies.listAttempts(input.operationId);
    const submitted = [...attempts].reverse().find(attempt => {
      if (attempt.status !== "submitted" || !attempt.usage) return false;
      const usage = attempt.usage as Record<string, unknown>;
      return typeof usage.providerAudioUrl === "string";
    });
    if (submitted) {
      const usage = submitted.usage as Record<string, unknown>;
      voice = {
        audioUrl: String(usage.providerAudioUrl),
        provider: claims.provider,
        voice: claims.voice,
      };
    } else {
      // A durable prepared attempt with no provider result after the original
      // request has returned to us is the crash window between provider
      // success and recording its URL. The provider offers no result lookup or
      // documented idempotency key, so the only safe recovery is to freeze the
      // operation as submission-unknown instead of leaving it pending forever
      // or risking a second paid submission.
      const prepared = [...attempts]
        .reverse()
        .find(attempt => attempt.status === "prepared");
      const preparedAt = prepared
        ? new Date(prepared.updatedAt ?? prepared.createdAt).getTime()
        : Number.NaN;
      if (
        prepared &&
        Number.isFinite(preparedAt) &&
        now - preparedAt >= NARRATION_PREPARED_RECOVERY_GRACE_MS
      ) {
        const message =
          "上一次旁白请求可能已经提交，但结果未能确认；已冻结操作且不会自动重试";
  ...
  if (!voice) {
    ...
    await dependencies.recordAttempt({
      ...
      model: claims.voice,
      status: "submitted",
      usage: { ...provenance, providerAudioUrl: voice.audioUrl },
    });
  }
  ...
  if (imported.status !== "ready") {
    ...
    return {
      status: "error",
      message: `旁白已生成，但受管音频导入失败：${imported.reason}`,
    };
  }
  await dependencies.recordAttempt({
    operationId: input.operationId,
...
export async function discardStoryNarrationCandidate(input: {
  ...
  return withVisualEditServiceLock(input.storyId, input.userId, async () => {
    ...
    // Adoption and discard must observe one serial Story state: otherwise a
    // clip can be committed after this reference check while discard removes
    // the same asset's managed bytes.
    const asset = await loadReadyStoryAudioAsset({
      scope: { storyId: input.storyId, userId: input.userId },
      assetId: input.candidateAssetId,
    });
