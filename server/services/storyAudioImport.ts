...
export async function fetchTrustedAudioBytes(
  ...
  const redirectLimit =
    options.redirectLimit ?? DOWNLOAD_DEFAULTS.redirectLimit;
...
const AUDIO_OPERATION_ID_PATTERN = /^[A-Za-z0-9_-]{1,80}$/;
export const MAX_LOCAL_AUDIO_BYTES = 16 * 1024 * 1024;
const MAX_USER_MANAGED_AUDIO_BYTES = 512 * 1024 * 1024;
const MAX_PROCESS_MANAGED_AUDIO_BYTES = 2 * 1024 * 1024 * 1024;
const MAX_GLOBAL_LOCAL_IMPORTS = 2;
const activeLocalImportsByUser = new Map<number, number>();
let activeLocalImports = 0;

/** Rejects excess work instead of queuing unbounded ffprobe/upload jobs. */
export function acquireLocalAudioImportPermit(
  userId: number
): (() => void) | null {
  if (
    activeLocalImports >= MAX_GLOBAL_LOCAL_IMPORTS ||
    (activeLocalImportsByUser.get(userId) ?? 0) >= 1
  ) {
    return null;
  }
  activeLocalImports += 1;
  activeLocalImportsByUser.set(userId, 1);
  let released = false;
  return () => {
    if (released) return;
    released = true;
    activeLocalImports = Math.max(0, activeLocalImports - 1);
    activeLocalImportsByUser.delete(userId);
  };
}

async function managedFileBytes(paths: readonly string[]): Promise<number> {
  let total = 0;
  for (const filePath of paths) {
    try {
      ...
      // Missing files are handled by the asset/recovery paths, not quota math.
    }
  }
  return total;
}

async function assertManagedAudioCapacity(
  userId: number,
  incomingBytes: number
): Promise<void> {
  if (incomingBytes <= 0 || incomingBytes > MAX_LOCAL_AUDIO_BYTES) {
    throw new Error("音频文件不能超过 16MB");
  }
  await ensureManagedAudioDirs();
  const userAssets = await listStoryAudioAssetRowsForUser(userId);
  const userBytes = await managedFileBytes(
    userAssets.map(asset => resolveManagedAudioPath(asset.storageKey))
  );
  if (userBytes + incomingBytes > MAX_USER_MANAGED_AUDIO_BYTES) {
    throw new Error("当前账号的受管音频已达 512MB 上限");
  }
  const entries = await readdir(managedAudioRoot(), { withFileTypes: true });
  const processBytes = await managedFileBytes(
    entries
      .filter(entry => entry.isFile())
      .map(entry => path.join(managedAudioRoot(), entry.name))
  );
  if (processBytes + incomingBytes > MAX_PROCESS_MANAGED_AUDIO_BYTES) {
    throw new Error("服务器音频存储已达上限，请稍后处理");
  }
}
...
export type ImportAudioBytesInput = {
  ...
  provenance?: unknown;
  ffprobePath?: string;
  /** Remote entry points freeze identity before downloading so settled replays do not fetch. */
...
function localImportRequestDigest(input: ImportAudioBytesInput): string {
  return (
    input.requestDigest ??
    requestDigest({
...
function invalidOperationIdResult(): StoryAudioImportResult {
  return {
    status: "failed",
    reason: "导入操作 ID 格式无效",
    failureCode: "invalid-operation-id",
  };
}
...
export async function importAudioBytes(
  ...
  if (!AUDIO_OPERATION_ID_PATTERN.test(operationId)) {
    return invalidOperationIdResult();
  }
  const digest = localImportRequestDigest(input);
  ...
  if (existing) {
    if (existing.requestDigest !== digest) {
      return {
        status: "failed",
        reason: "这个导入操作 ID 已经用于不同的请求",
        failureCode: "operation-payload-conflict",
      };
    }
    ...
    return {
      status: "failed",
      reason: "这次导入仍在处理，请稍后查看",
      failureCode: "import-in-progress",
    };
  }
  ...
  if (input.sourceKind === "local-upload") {
    try {
      await assertManagedAudioCapacity(scope.userId, input.bytes.byteLength);
    } catch (error) {
      return {
        status: "failed",
        reason: error instanceof Error ? error.message : "音频存储额度不足",
        failureCode: "audio-quota",
      };
    }
  }
  ...
  const bundle = await createStoryAudioImportBundle({
    asset: {
      storyId: scope.storyId,
      userId: scope.userId,
      storageKey,
  ...
  if (!bundle.created) {
    if (bundle.operation.requestDigest !== digest) {
      return {
        status: "failed",
        reason: "这个导入操作 ID 已经用于不同的请求",
        failureCode: "operation-payload-conflict",
  ...
  let ready: StoryAudioAsset | null;
  try {
    ready = await markStoryAudioAssetReady({
      scope,
      assetId: asset.id,
      probe,
      checksum,
  ...
  try {
    ...
    // The asset row + managed file are already durable. Recovery recognizes
    // this combination and finalizes the operation instead of invalidating it.
  }
...
export async function materializeRemoteAudio(
  ...
  if (!AUDIO_OPERATION_ID_PATTERN.test(input.operationId)) {
    return invalidOperationIdResult();
  }
 ...
 * Timeline.
 */
export async function recoverStaleAudioImports(
  options: {
    now?: number;
    graceMs?: number;
  } = {}
  ...
  for (const op of unsettled) {
    ...
    const scope = { storyId: op.storyId, userId: op.userId };
    const asset =
      op.assetId == null
        ? null
        : await getStoryAudioAssetRow({
            assetId: op.assetId,
            storyId: op.storyId,
            userId: op.userId,
          });
    const managedPath = asset
      ? resolveManagedAudioPath(asset.storageKey)
      : null;
    const stagingPath = resolveAudioStagingPath(
      op.stagingKey ?? op.operationId
    );
    const exists = async (candidate: string | null) => {
    ...
    if (asset?.status === "ready" && managedExists) {
      await updateStoryAudioImportOperationRow(op.id, { status: "ready" });
      continue;
    }

    const recoverablePath = managedExists
      ? managedPath
      : stagingExists
        ? stagingPath
        : null;
    if (asset && asset.status === "pending" && recoverablePath) {
      try {
        const probe = await probeStagedAudio(recoverablePath);
        const checksum = checksumBytes(await readFile(recoverablePath));
        if (!managedExists) {
          await rename(recoverablePath, managedPath!);
        }
        ...
        // Fall through to compensation. A non-ready managed file is removed.
      }
    }

    await discardAudioStagingFile(op.stagingKey ?? op.operationId);
    if (asset) {
