...
async function runSubtitleCommand(
  ...
  options: {
    markBoundNarrationStale?: boolean;
    initializeMissingTimeline?: boolean;
  } = {}
  ...
  return withVisualEditServiceLock(input.storyId, input.userId, async () => {
    ...
    for (let attempt = 0; ; attempt += 1) {
      let envelope = await loadOwnedStoryTimelineEnvelope({
        storyId: input.storyId,
        userId: input.userId,
      });
      let createsTimeline = false;
      if (!envelope && options.initializeMissingTimeline) {
        ...
        // A Timeline may have appeared between the first read and this owned
        // aggregate read. Retry from the raw stored envelope instead of
        // normalizing and rewriting an existing visual document.
        if (aggregate.timeline) continue;

        // The editor already projects a missing Timeline from Story shots.
        // Persist that exact visual projection together with the first
        // subtitle slice so old Stories gain one canonical Timeline without
        // ever passing a transient, visually empty row through the system.
        const projected = projectStoryTimelineDocument(aggregate.story, null);
        envelope = {
          version: 0,
          items: projected.items,
          overlays: projected.overlays,
          visualLayerState: projected.visualLayerState,
          extensions: {},
        };
      ...
      try {
        const persistedExtensions = appendDurableTimelineMediaOperation(
          { ...envelope.extensions, ...nextExtensions },
          {
            ...input.operation,
            commandDigest,
          }
        ...
        const saved = await saveStoryTimelineExtensionCas({
          ...
          ...(createsTimeline
            ? {
                currentOverlays: envelope.overlays,
                currentVisualLayerState: envelope.visualLayerState,
              }
            : {}),
...
export function initializeSubtitlesForStory(input: {
  ...
  return runSubtitleCommand(
    ...
    { initializeMissingTimeline: true }
  );
}
  ...
  return withVisualEditServiceLock(input.storyId, input.userId, async () => {
    ...
    try {
      ...
      const restoredSlices: Record<string, unknown> = {
        ...
        [AUDIO_SLICE_KEY]: entry.beforeExtensions[AUDIO_SLICE_KEY] ?? undefined,
      };
      const saved = await saveStoryTimelineExtensionCas({
        storyId: input.storyId,
        userId: input.userId,
