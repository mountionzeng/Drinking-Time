...
function overlayCandidateMatchesCanonical(
  ...
  if (allowTimelineVersionRefresh) {
    return (
      ...
      JSON.stringify(requested.placement) ===
        JSON.stringify(canonical.placement) &&
...
async function finishAndApply(params: {
  ...
  try {
    if (candidate.placement?.kind === "timeline-overlay") {
      ...
      const canonical = normalizeCandidate(refreshed.proposal);
      candidate = {
        ...candidate,
        expectedTimelineVersion: canonical.expectedTimelineVersion,
        source: { ...candidate.source, imageUrl: canonical.source.imageUrl },
        target: { ...candidate.target, imageUrl: canonical.target.imageUrl },
      };
    }
