...
function analysisSystemPrompt(kind: VisualAssetKind): string {
  ...
  return [
    ...
    ...(kind === "scene"
      ? [
          "画面里的人物、他们的长相、服装和姿态**绝不属于场景固定事实**：人物是另一套独立资产，由每个镜头各自安排。fixedProps 只写这个空间里长期固定存在的物件，例如家具、装置、灯具、墙上挂画、乐器、展台。",
          "同一个空间从不同机位拍摄时，同一件东西在画面里的位置会变。**位置差异不是冲突**；只有这件东西本身的形态、材质或存在与否不一致时才算冲突（例如「嵌在墙面」与「立在地面」是冲突，「在画面左上」与「在画面中央」不是）。",
        ]
      : []),
    ...(kind === "style"
      ? [
          "只提取画法本身：媒介、笔触、造型语言、色彩语言、禁忌。画的是谁、画的是什么空间，都不属于风格固定事实。",
        ]
      : []),
...
export const CANONICAL_BOARD_COMPOSITION_VERSION = 6;
...
const VIEW_BRIEF: Record<VisualAssetView["role"], string> = {
  ...
  establishing:
    "主视角全景：一张交代整个空间的大全景。要看得见地面、至少两面墙的交界和天花板关系，让人能读懂房间的形状和主要陈设的相对位置。",
  reverse:
    "反向视角：相机原地旋转 180°，站在主视角相机的位置往回看。**必须画出主视角里位于镜头背后、看不到的那一面**——那面墙、那些陈设。这不是主视角的另一张构图，也不是换个角度的同一面；如果画出来的还是主视角看得见的那面墙，就是错的。空间几何、材质和固定陈设必须与主视角完全一致。",
  side:
    "侧向视角：相机从主视角原地旋转 90°，沿垂直于主视角的方向看同一空间。要能看出房间的进深关系，和主视角互为直角。",
  top:
    "正交俯视平面图：相机垂直悬在空间正上方向下拍，视线与地面成 90°。看到的是地面的平面布局——墙的围合形状、固定陈设的俯视轮廓和它们的间距。不是斜俯视，不是高角度透视，画面里不应出现墙面的立面。",
  "character-sample":
    "人物样例：用这套美术风格画一个人物全身或半身。主体内容可以和参考图完全不同，只有媒介、笔触、造型语言和色彩语言必须一致。",
  "scene-sample":
    "场景样例：用这套美术风格画一处有纵深的场景。主体内容必须和上一格的人物完全不同，只有风格语言一致。",
  "object-sample":
    "物件样例：用这套美术风格画一件静物。主体内容必须和前两格完全不同，只有风格语言一致。",
  "closeup-sample":
    ...
    ...(asset.kind === "character"
      ...
      : asset.kind === "scene"
        ? [
            ...
            // 把空间抽成一个悬空物件或空影棚（2026-08-22 发现，花钱之前修掉）。
            "这是一张空间的实景图，环境本身就是画面主体：地面、墙面、天花板、固定陈设都要在画面里，不要抠成白底或中性影棚。",
            "画面里不要出现任何人物或动物——标准视图只锁空间，人物由各镜头自己安排。",
            "相机必须真的移动到本视角要求的位置，不要沿用参考图的机位。空间几何、材质和固定陈设保持不变，变的只有机位。",
            // 2026-08-22 画廊首版：模型自己加了一个黑色底座+白板，固定事实里没有。
            // 「不要额外元素」说得不够死，必须声明清单是封闭的、空房间可接受。
            // 「宁可画成空房间」是 2026-08-22 为堵住模型凭空加黑底座写的，
            // 但对「什么都有、大小悬殊」的杂陈空间是毒药 —— 它会一直给出空展厅。
            // 正确的约束是「照着固定事实的密度画」，而不是「越空越安全」。
            "只画固定事实里写明的东西，不要自行发明新的固定结构。但**必须照着固定事实描述的密度和丰富度去画**：固定事实说这里陈设密集、门类杂陈、尺度悬殊，就要真的把画面填满；说这里空旷简洁，才画成空房间。不要因为怕出错就把空间画空。",
            // 同一版四格里展台一会儿是居中岛台、一会儿是贴墙长条。
            "固定陈设在房间里的位置和形状是钉死的：同一件东西在四个视角里必须是同一个形状、同一个尺寸、贴着同一面墙或占着同一块地面，变的只有你看它的角度。不要因为换了机位就重新设计它。",
          ]
        : [
            "四格的主体内容必须彼此不同，这是一张风格样例板，不是同一张画的四个角度。",
            "不要沿用参考图的构图和主体，只沿用它的媒介、笔触、造型语言和色彩语言。",
          ]),
