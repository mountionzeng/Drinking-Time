...
function wrapSubtitleLines(
  ...
  return lines.flatMap(line => {
    ...
    for (let index = 0; index < characters.length; index += maxCharactersPerLine) {
      wrapped.push(characters.slice(index, index + maxCharactersPerLine).join(""));
    }
    return wrapped;
  });
}
...
export function buildTimelineMediaFfmpegPlan(input: {
  ...
  const inputPaths = [
    ...
    ...(input.subtitleOverlays ?? []).map(overlay => overlay.filePath),
  ];
  const graph: string[] = [];
  const videoMap = "[videoout]";
  const subtitleOverlays = input.subtitleOverlays ?? [];
    ...
    subtitleOverlays.forEach((overlay, index) => {
      ...
      graph.push(
        `[${previousLabel}][${inputIndex}:v:0]overlay=0:0:enable='gte(t,${frameSeconds(
          overlay.startFrame
        ).toFixed(6)})*lt(t,${frameSeconds(overlay.endFrame).toFixed(6)})'[${outputLabel}]`
      );
...
async function loadSubtitleFontBase64(
  ...
  try {
    const file = (await readdir(fontDirectory))
...
async function renderSubtitleOverlays(input: {
  plan: SubtitleRenderPlan;
  workDir: string;
  dimensions: { width: number; height: number };
  fontDirectory?: string | null;
}): Promise<ResolvedSubtitleOverlay[]> {
  const intervals = buildSubtitleOverlayIntervals(input.plan);
...
export async function composeTimelineMedia(input: {
  ...
  const subtitleOverlays = await renderSubtitleOverlays({
    plan: input.subtitlePlan,
    workDir: input.workDir,
    dimensions: input.dimensions,
    fontDirectory: input.subtitleFontDirectory,
  });
