...
function choose(
  ...
  const ranked = cards
    .map(card => ({ card, score: scoreCard(card, normalized) }))
  ...
  if (ranked[1] && best.score - ranked[1].score < MIN_MARGIN)
    return { card: null, reason: "ambiguous" as const };
...
export function selectSemanticArtDirection(args: {
  ...
  const cards = (args.cards ?? getSemanticArtCards()).filter(
    card => !card.forbiddenPurposes.includes(args.purpose)
  );
  const scores = Object.fromEntries(
    cards.map(card => [card.id, scoreCard(card, args.normalized)])
  );
  const mainChoice = choose(
    cards.filter(card => card.scope === "main"),
    args.normalized
  );
  ...
  const effectiveMain =
    mainChoice.card ??
    cards.find(
      card => card.id === args.currentMainId && card.scope === "main"
    ) ??
    null;
  let reason: SemanticArtSelection["reason"] =
    mainChoice.reason !== "applied"
      ? effectiveMain && auxChoice.reason === "applied"
        ? ("applied" as const)
        : mainChoice.reason
  ...
  const dimensionsAllowed =
    auxiliary && effectiveMain
      ? auxiliary.allowedAuxiliaryDimensions.every(dimension =>
          effectiveMain.allowedAuxiliaryDimensions.includes(dimension)
        )
      : false;
  if (
    auxiliary &&
    (!effectiveMain ||
      !dimensionsAllowed ||
      (auxiliary.compatibleMainIds.length > 0 &&
        !auxiliary.compatibleMainIds.includes(effectiveMain.id)))
  ) {
    auxiliary = null;
