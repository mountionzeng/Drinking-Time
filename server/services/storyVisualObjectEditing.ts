...
import {
  authorizeStoryVisualReferences,
  loadOwnedStoryVisualAggregate,
} from "../persistence/storyVisualPersistence";
import {
  getStoryMaterialState,
  projectStoryTimelineDocument,
} from "./storyMaterials";
import {
  runStoryTimelineCommand,
  type StoryTimelineCommandContext,
} from "./storyTimelineEditing";
...
> {
  ...
  const aggregate = await loadOwnedStoryVisualAggregate({
    storyId: input.storyId,
    userId: input.userId,
    includeVideoTakes: input.object.type === "story-shot",
  });
  if (!aggregate) return { status: "error", error: "故事不存在或无权访问" };
  ...
  const key = clipboardScope(
    input.userId,
    input.storyId,
    input.editorSessionEpoch
  );
...
function readClipboard(input: {
  ...
  return (
    clipboards
      .get(
        clipboardScope(input.userId, input.storyId, input.editorSessionEpoch)
      )
...
async function authorizeStoryShotSnapshot(input: {
  ...
  const snapshot = cloneVisualObjectClipboardSnapshot(
    input.snapshot
  ) as StoryShotClipboardSnapshot;
  const takeRefs = [
    ...(snapshot.timeline.primaryVideoEdit
      ? [{ takeId: snapshot.timeline.primaryVideoEdit.takeId, rangeId: null }]
      : []),
    ...snapshot.timeline.visualClips.map(clip => ({
      takeId: clip.takeId,
      rangeId: clip.rangeId,
    })),
  ];
  const authorization = await authorizeStoryVisualReferences({
    storyId: input.storyId,
    userId: input.userId,
    referencedImageId: snapshot.timeline.referencedImageId ?? null,
    takeRefs,
  });
  if (!authorization) return null;
  const takeById = new Map(
    authorization.videoTakes.map(take => [take.id, take] as const)
  );
  return {
    ...snapshot,
    timeline: {
...
type AggregateCommandInput = {
  ...
  planner: (context: StoryTimelineCommandContext) =>
    ...
    | {
        ...
        document: StoryTimelineCommandContext["document"];
      };
};
...
async function runAggregateCommandUnlocked(
  input: AggregateCommandInput
): Promise<StoryVisualObjectEditResult> {
  if (
    !isVisualEditSessionEpochAllowed({
  ...
  const command = await runStoryTimelineCommand(
    {
      ...
      ...(input.legacyOverlays?.length
        ? { legacyOverlays: input.legacyOverlays }
        : {}),
    },
    input.planner
  );
  ...
  const identityFingerprint = digest({
    shots: shotsOf(command.facts.after.storyBody).map((shot, index) =>
...
async function pasteStoryVisualObjectUnlocked(input: {
  ...
  const snapshot = await authorizeStoryShotSnapshot({
    ...input,
    snapshot: raw,
  });
  if (!snapshot)
    return {
      status: "error",
      error: "剪贴板引用的素材已失效或无权访问",
      errorKind: "invalid",
    };
  const stableShotId = deterministicId("paste-shot", {
    ...input.operation,
    storyId: input.storyId,
  });
  const ownedIds = snapshot.timeline.visualClips.map((_, index) =>
    deterministicId("paste-owned", {
      ...input.operation,
      storyId: input.storyId,
      index,
    })
  ...
  return runAggregateCommand({
    ...
    planner: context => {
      const plan = pasteStoryShotClipboardSnapshot({
        aggregate: {
          shots: shotsOf(context.storyBody),
...
async function splitStoryVisualShotUnlocked(input: {
  ...
  const rightStableShotId = deterministicId("split-shot", {
    ...input.operation,
    storyId: input.storyId,
    stableShotId: input.stableShotId,
  ...
  return runAggregateCommand({
    ...
    planner: context => {
      ...
      const itemIndex = baseDocument.items.findIndex(
        item => item.stableShotId === input.stableShotId
      );
      const row = buildTimelineLayout(baseDocument.items).find(
        candidate => candidate.item.stableShotId === input.stableShotId
      );
      if (shotIndex < 0 || itemIndex < 0 || !row)
        return { status: "error", message: "镜头不存在或已经更新" };
      const sourceItem = baseDocument.items[itemIndex];
      const timeline = splitTimelineItem({
        item: sourceItem,
      ...
      if (timeline.kind === "blocked")
        return { status: "error", message: timeline.reason };
      const leftImages = (sourceItem.imageClips ?? [])
        .filter(
          clip =>
            (clip.timelineStartFrame ?? row.startFrame + clip.offsetFrames) <
            input.cutFrame
        )
        .map(clip => {
          const start =
          ...
          return {
            ...
            offsetFrames: start - row.startFrame,
          };
        });
      const rightImages = (sourceItem.imageClips ?? [])
        .filter(
          clip =>
            (clip.timelineStartFrame ?? row.startFrame + clip.offsetFrames) >=
            input.cutFrame
        ...
        .map(clip => {
          ...
          return {
            ...clip,
            timelineStartFrame: start,
            offsetFrames: start - input.cutFrame,
          };
        });
      ...
      const story = splitStoryShotAtIndex({
        shots,
        index: shotIndex,
        rightStableShotId,
        leftDurationMs: left.plannedDurationMs,
        rightDurationMs: right.plannedDurationMs,
      });
      if (!story) return { status: "error", message: "镜头拆分失败" };
      const items = [
        ...baseDocument.items.slice(0, itemIndex),
        left,
        right,
        ...baseDocument.items.slice(itemIndex + 1),
      ].map((item, position) => ({ ...item, position }));
      return {
        status: "ok",
        value: { rightStableShotId },
        storyBody: { ...context.storyBody, shots: story.shots },
        document: { ...baseDocument, items },
      };
    },
  });
}
...
export function storyVisualClipboardSizeForTesting(input: {
  storyId: number;
  userId: number;
  editorSessionEpoch: string;
}): number {
...
export function retireStoryVisualClipboardScope(input: {
  storyId: number;
  userId: number;
  editorSessionEpoch: string;
}): void {
