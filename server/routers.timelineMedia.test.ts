...
} from "./services/visualEditSessionRegistry";
...
describe("timelineMedia router", () => {
  it("quotes generated audio from the owned shot at the playhead", async () => {
    const caller = appRouter.createCaller(context(700));
    const storyId = await seedStory(700);

    const quote = await caller.timelineMedia.quoteSceneAudio({
      storyId,
      kind: "music",
      targetFrame: 0,
      intent: "轻一点，不要盖住旁白",
    ...
    expect(quote).toMatchObject({
      kind: "music",
      context: { stableShotId: "shot-a", startFrame: 0 },
      currency: "CNY",
    });
    expect(quote.prompt).toContain("轻一点，不要盖住旁白");
    expect(quote.quoteToken.length).toBeGreaterThan(40);
  });
  ...
  it("blocks stale-session import, paid narration submission, and candidate discard before mutation", async () => {
    const caller = appRouter.createCaller(context(715));
    const storyId = await seedStory(715);
    expect(
      activateVisualEditSession({
        storyId,
        userId: 715,
    ...
    ).toMatchObject({ status: "ok", replacedEpoch: "epoch-old" });

    const candidate = await createStoryAudioAssetRow({
      storyId,
      userId: 715,
      storageKey: "f".repeat(32),
      displayName: "candidate.mp3",
      sourceKind: "tts",
      status: "ready",
      durationFrames: 30,
      provenance: {
    ...
    await expect(
      caller.timelineMedia.importLocalAudio({
        storyId,
        operation,
        fileName: "stale.wav",
        fileBase64: silentWavBase64(),
    ...
    await expect(
      caller.timelineMedia.generateNarrationCandidate({
        storyId,
        operation,
        subtitleCueId: "cue-a",
      ...
      status: "error",
      message: expect.stringContaining("会话已经失效"),
    });
    await expect(
      caller.timelineMedia.discardNarrationCandidate({
        storyId,
        operation,
    ...
    expect(await listStoryAudioAssetRows({ storyId, userId: 715 })).toEqual([
