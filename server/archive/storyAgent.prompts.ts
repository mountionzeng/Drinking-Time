...
export function buildAgentSystemPrompt(
  ...
  return [
    ...
    ...(enableImageGen ? [
      ...
      "你只要在 reply 里自然地把这句提议说出来就好 —— 具体的出图指令由后台那一步来记，你这一轮不用写任何 JSON 或工具调用。",
    ] : []),
    "",
    "【这一轮只输出一段话】",
...
export function buildCardExtractionPrompt(
  ...
  return [
    ...
    "读下面这段对话（重点是对方【最后一轮】说的话，以及小酌刚刚给出的回应），把这一轮的情绪信号抽成结构化数据。",
    "",
    ...
    ...(enableImageGen ? [
      "",
      "toolCalls：只有当小酌在回应里真的提议了出图（如「我帮你画一个画面？」），或对方描述了一个足够具体、情绪到位的场景时，才加一条 generateImage；否则给空数组 []。",
