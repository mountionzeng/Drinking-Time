...
function formatFictionIntentBlock(intent?: StoryChatIntentPayload): string {
  ...
  return [
    "【当前创作模式：创造另一个世界 / 虚构短片】",
    "你现在不是求职影片顾问，也不是私人回忆陪聊，而是在帮用户把一句虚构灵感发展成一套能直接生成 Storyboard 表格的短片故事。",
    "优先任务顺序：",
...
export function buildAgentSystemPrompt(
  ...
  const pacing = (() => {
    if (isJobSearch) {
      return [
        "【求职模式节奏】",
        "不要因为已有一些素材就停止收集；求职片需要目标职位、简历项目、成果数字、优势证据、顾虑/限制都比较清楚。",
        "用户的对话原话会直接进入 Storyboard 来源；不要让用户先整理或确认故事卡。",
      ].join("\n");
    }
    if (isFiction) {
      return [
    ...
    if (directToStoryboard) {
      return [
        "【确认意图后的节奏】",
        "用户的对话原话会直接作为 Storyboard 表格来源。继续接住真实细节，一次只补一个最关键的信息。",
        "不要向用户提故事卡、卡片确认或出卡；信息够用时，引导他点击“直接生成 Storyboard 表格”。",
      ].join("\n");
    }
  ...
  const taskBlock = (() => {
    if (isJobSearch) {
      return [
        "你在做的事很具体：帮对方把求职目标、简历经历、项目证据和个人定位整理成能打动招聘者、可直接生成 Storyboard 表格的短片素材。",
        "对话里可以包含一条 JD 要求、一段简历经历、一个项目职责、一个量化结果、一个作品链接、一个能力组合、一个证据缺口或定位顾虑。",
        "请尽量保留用户自己的岗位词、项目事实、数字、工具方法和作品线索，不要替他编经历、夸大成果或空泛包装。",
      ].join("\n");
    }
    if (isFiction) {
      return [
        "你在做的事很具体：帮对方把一句虚构灵感补成能直接生成 Storyboard 表格的短片故事。",
        "对话需要能支持一个 3-5 镜短片：故事核心、主角/视点、欲望、阻碍、关键场景、视觉风格和余味都要尽量明确。",
        "请保留用户原始灵感里的怪味、规则和画面，不要把它改成求职优势、真实经历复盘或空泛世界观百科。",
      ].join("\n");
    }
    if (directToStoryboard) {
      return [
        "你在做的事很简单：陪对方把要讲的内容说清楚，并保留他的原话和个人痕迹。",
        "确认意图后，这些对话会直接成为 Storyboard 表格的生成来源，不再经过用户可见的故事卡。",
        "不要替他解释、升华或加 moral；信息够用时，引导他点击“直接生成 Storyboard 表格”。",
      ].join("\n");
    }
    return [
  ...
  return [
    ...
    "【固定机制 · 每轮都要遵守】",
    directToStoryboard
      ? "1. 意图已确认：绝不要在 reply 里提故事卡、出卡或卡片确认。对话原话会直接作为 Storyboard 表格来源。"
