...
import type { SimilarStoryCardPayload, ShotDraft } from "./storyAgent.types";
...
export function buildAgentSystemPrompt(
  ...
  return [
    ...
    // 所以当有照片时，显式把注意力先拉到图上。
    // ⚠️ 顺序很关键：第一句先钉死「格式不变、话写进 reply」，再讲怎么看图——
    //    否则模型一上来被「自然回应」带跑，就容易脱壳直接说人话 → parseJsonLoose 失败 →
    //    card 永远为 null（「能识别图、但一直不出卡」就是这么来的）。
    //    即便它真破功，上层还有 json_object 模式 + 一次「只要 JSON」重试兜底（见 replyFromStoryAgent）。
    ...(photoShared ? [
      "【这一轮对方分享了一张照片】",
      "格式不变：这一轮**照样**只返回结尾约定的那套严格 JSON（read / reply / card）。你对照片说的所有话都写进 reply 字段里，绝不脱离 JSON 直接说人话。",
      "在这个前提下，先真的去看这张图：是什么场景、什么光线和氛围、有没有人、表情和姿态、桌上窗外有什么、最先抓住你视线的是哪个细节。",
      "然后在 reply 字段里，像朋友看到你发来的照片那样自然回应——说出你在图里看到的那个具体的东西，再顺着它往下聊或轻轻问一句；语气是聊天，不是「图像识别报告」式地罗列清单。",
