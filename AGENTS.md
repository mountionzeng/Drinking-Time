...
## 功能生命账本（2026-08-11 起生效）

1. 开始修改前先查 `docs/features/feature-ledger.json`，向用户说明相关已有功能、状态、不可破坏约束和已知缺口。
2. 如果新工作可能覆盖、削弱、替换或删除已登记功能，**必须停止修改**，列出影响与取舍，等待用户明确批准。
3. 持久的用户能力建立功能卡；修复、重构和性能优化追加到所属功能的 `history`，不另建噪音卡。
4. 完成后更新功能状态、权威代码、测试证据、依赖和缺口，并运行 `pnpm feature:validate`。
5. 页面、类型或计划存在不等于功能可用；`working` 必须有真实入口和可执行测试/验证证据。

详细字段和状态定义见 `docs/features/README.md`。
